...
then to feed those features to our classifier for training  it in order to disciminate classes. This solution includes the following steps:
\begin{enumerate}
...
According to Table \ref{table:svm_mod_roialign}, modified Roi Align doesn't improve mAP performace. On the contrary, it reduces it.
However, the gap between those 2 approaches is small, so we don't abandon this idea, becuase, for different approaches,
...
maps because it wouldn't performed better that (256,8,7,7) ferature maps as noticed from the previous section. \par
We experiment using a SVM classifier for training policies 1 and 4 and using both regular and modifier Roi Align. The
perfomance results are presented at Table \ref{table:svm_temp_pooling}.
...
However, the difference between those performaces is about 2\%. Probably, this is caused by the fact that, in the temporal pooling approach,
...
in which SVM has to train half the number of parameters. Futhermore, comparing above results with results shown at Table  \ref{table:svm_mod_roialign}, we can see that we get about the same results for both approaches. So, we choose to keep using approach with sample duration equal
...
\begin{center}
  ...
  \begin{longtable}{|| c | c | c | c || c c c||}
    ...
    \hline
    \multirow{2}{*}{\textbf{F. map}} & \multirow{2}{*}{\textbf{FG tubes}} & \multirow{2}{*}{\textbf{Total tubes}} & \multirow{2}{*}{\textbf{Policy}} & {} & \textbf{mAP} & {} \\
    {}  & {} & {} & {} & 0.5 & 0.4 & 0.3 \\
    \hline
    \multirow{8}{*}{(2,256,8,7,7)} & \multirow{2}{*}{1} & \multirow{2}{*}{3} & 1 & 25.07 & 26.91 & 29.11\\
    \cline{4-7}
    {} & {} & {} & 4 & 23.27 & 25.96 & 28.25\\
    \cline{2-7}
    {} & \multirow{2}{*}{3} & \multirow{2}{*}{8} & 1 &  24.38 & 25.97 & 26.4 \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
    \cline{2-7}
    {} & \multirow{6}{*}{4} & \multirow{2}{*}{8} & 1 &  Pending... \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
    \cline{3-7}
    {} & {} &  \multirow{2}{*}{12} & 1 & Pending... \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
    \cline{3-7}
    {} & {} &  \multirow{2}{*}{16} & 1 & Pending... \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
    \cline{2-7}
    {} & \multirow{6}{*}{8} & \multirow{2}{*}{16} & 1 &  Pending... \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
    \cline{3-7}
    {} & {} &  \multirow{2}{*}{24} & 1 & Pending... \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
    \cline{3-7}
    {} & {} &  \multirow{2}{*}{32} & 1 & Pending... \\
    \cline{4-7}
    {} & {} & {} & 4 & Pending... \\
...
\begin{center}
  \begin{longtable}{|| c | c || c c c ||}
    ...
    {} & {} & 0.5 & 0.4 & 0.3 \\
    \hline
    \multirow{3}{*}{4} & 8 & Pending... \\
    \cline{2-5}
    {} & 12 & Pending... \\
    \cline{2-5}
    {} & 16 & Pending... \\
    \hline
    \multirow{3}{*}{8} & 16 &  12.29 & 19.51 & 23.11  \\
    \cline{2-5}
...
\begin{center}
  \begin{longtable}{|| c | c || c c c ||}
    ...
    {} & {} & 0.5 & 0.4 & 0.3 \\
    \hline
    \multirow{3}{*}{4} & 8 & 15.11 & 19.78 & 21.14 \\
    \cline{2-5}
    {} & 12 & 11.39 & 15.74 & 18.15 \\
    \cline{2-5}
    {} & 16 & 13.62 & 16.11 & 18.15 \\
    \hline
    \multirow{3}{*}{8} & 16 & 12.98 & 17.52 & 19.10 \\
    \cline{2-5}
...
\item After Residual Layers, we perfom temporal pooling for x-y dimensions. So we get as output activation maps with dimension size
  equal with (512,).  Finally, we feed these feature maps to a linear layer in order to get class condifence score, after applying
  soft-max function.
...
and 8. Futhermore, we will train it for 3 different ratios between the number of foreground and background action tubes, which are 1:1, 1:2
...
\begin{center}
  \begin{longtable}{|| c | c || c c c ||}
    ...
    {} & {} & 0.5 & 0.4 & 0.3 \\
    \hline
    \multirow{3}{*}{4} & 8 & 5.89 & 9.54 & 13.61 \\
    \cline{2-5}
    {} & 12 & 9.51 & 12.8 & 14.6  \\
    \cline{2-5}
    {} & 16 & Pending... \\
    \hline
    \multirow{3}{*}{8} & 16 & Pending... \\
    \cline{2-5}
    {} & 24 & Pending... \\
    \cline{2-5}
    {} & 32 & Pending... \\
...
\begin{figure}[h]
  % \includegraphics[scale=0.7]{convolutional_neural_network_structure} \]
  \centering
  \includegraphics[scale=0.4]{model_nms}
...
got score about 92-94\%. So, we came with idea of just performing temporal localization, instead of spatio-temporal localization. \par
...
\begin{enumerate}
\item Fist, we use TPN in order to propose  spatio-temporal ToIs, just like we did in previous approaches. Then, we link those ToIs based
...
\item Previous steps is exactly the same as previous classification approaches. However, in this approach, we don't use any kind of Roi Align
  ...
  spatio-temporal and temporal nms is the overlapping criterion, which is used. For spatio-temporal nms, we use spatiotemporal IoU and
  respectively, for temporal we use temporal IoU as presented in chapter 2.
\item Of course, the proposed action tubes last more that 16 frames, which we set as sample duration. So, we seperate action tubes into
...
\item So, for each video clip, for each class we get a confidence score after performing softmax operation. Finally, we get
  average confidence score for each class, and we consider the best-scoring class as the class label of each actio tube.
...
\begin{center}
  ...
  \begin{longtable}{|| c | c || c c c ||}
    ...
    {} & {} & 0.5 & 0.4 & 0.3\\
    \hline
    \multirow{3}{*}{0.9} & {0.75} & 0.20 & 0.25 &0.30 \\
    \cline{2-5}
    {} & {0.} & 0.38 & 0.45 & 0.54 \\
    \cline{2-5}
    {} & {0.85} & 0.49 & 0.55 & 0.64 \\
    \hline
    \multirow{3}{*}{0.7} & {0.75} & Pending...\\
    \cline{2-5}
    {} & {0.8} & Pending...\\
    \cline{2-5}
    {} & {0.9} & Pending...\\
    \hline
    \multirow{3}{*}{0.5} & {0.75} & Pending...\\
    \cline{2-5}
    {} & {0.8} & Pending...\\
    \cline{2-5}
    {} & {0.9} & Pending...\\
    ...
    \caption{UCF's temporal localization mAP perfomance}
