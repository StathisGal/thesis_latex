...
in chapter 5, it is clear that our network would fail recognizing most of spatiotemporal action tubes and to correctly classify them. However, inn most cases, MABO performance
got score about 92-94\%. So, we came withthe idea of rather than performing spatiotemporal localization and
classification, for dataset UCF, we perform only temporall localization. This means that  we tried to detect video segments in which an action is performed, and also we try
to determine the class of the performed action.


\subsection{Only temporal classification}

As presented in chapter 5, our connection algorithm is able to get good temporal recall and MABO performance. 
