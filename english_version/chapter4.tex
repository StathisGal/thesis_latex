...
\begin{itemize}
\item One 3D Convolutional Network, which is used for feature extraction. In our implementation we use a 3D Resnet network which is taken from
...
Our architecture gets as input a sequence of frames which has a fixed  width, height and duration. However, each video has different resolution. That's creates the
need to resize each frame before feeding it to the architecture.
As mentioned in previous chapter, the first element of our network is a 3D ResNet taken from \cite{hara3dcnns}. This network is designed to
...
corresponding layers, we use Non-Maximum Suppression (NMS) algorithm.  We set NMS threshold equal with 0.7,
...
it considers x-y dimensions to have same importance with time dimension, which we do not desire. That's because
...
After first test, we came with the idea that in a video lasting 16 frames, in time domain, all kind of actions can be separated in the following categories:
\begin{enumerate}
...
On top of that, we noticed that most of actions, in our datasets, last more that 16 frame. So, we came with the idea to add  1 scoring layer and 1 regression layer
...
\begin{figure}[h]
  ...
  \label{fig:tpn_1_2}
\end{figure}
Our goal is to ``compress'' feature maps in temporal dimension in order to propose ToIs based only on the spatial information.
...
\par
Training and Validation procedures remain the same. The only big difference is that now we have losses obtained from 2 difference system proposed TOIs. So, during validation, we,at first, concate
...
The only problem is that the regressor needs as input a fixed size feature maps. This problem is already solved by R-CNNs which use roi pooling and roi align
...
For each TOI whose activation map has size equal with (64,16,7,7), we get as output a feature map with size (64, 16, 7, 7). \par
...
use any pooling operation before feeding the first 2D Convolutional Layer. On the contrary, we manipulate our feature maps like not being spatiotemporal but,
...
is gets far better results from the others approaches, followed by approach with sample duration = 4. For UCF dataset, although sample duration equal with 8 gives us best performances
...
Our goal is bounding boxes to overlap with the actor precisely. According to Figure \ref{fig:proposals}, all of three proposed actions tubes
presented at Figures \ref{fig:proposals0}, \ref{fig:proposals1} and \ref{fig:proposals2} overlap with the actor to some degree.
However, by looking, it is clear that ToI, presented at Figure \ref{fig:proposals1}, doesn't overlap very well like those appearing at Figures \ref{fig:proposals0} and \ref{fig:proposals2}
...
However, as shown at Figures \ref{fig:proposals_gt0} and \ref{fig:proposals_gt1}, the second ToI better overlaps with the groundtruth. It is clear that even though
