...
\begin{enumerate}
\item Firstly,  initialize empty lists for the final tubes, final tubes' duration, their scores, active tubes, their correnspondig duration,
...
\item Finally, as we mentioned before, we have a list which contains the indexes of the saved tubes. So, we modify them in order to have
  the corresponding bounding boxes. However, 2 succeeding ToIs do not, always, have extactly the same bounding boxes in the frames that overlap. For example,
...
most operations are, from now on, matrix operations. On top that, last step of the algorith, which is the modification from indices to actual action tubes is written, now, in CUDA code so
...
\item [ Progress rate, ] which is defined as the progress proportion that each action class has been perfomed.
...
In this approach, each class is handled seperately, so for the rest section, we discuss action tube generation for one class only. In order to link 2 TOIs, for
...
From the above table, firstly, we corfirmed that when video step is equal with sample duration gives us the best recall results.
...
\begin{figure}[h]
  ...
  \centering
  \includegraphics[scale=0.25]{tube_ex2_half}
  \caption{Example of connected tubes}
  \label{fig:tube_ex}
\end{figure}

Figure \ref{fig:tube_ex} shows the example presented in chapter 3, after linking first video segment's first ToI with a ToI proposed for the second video segment.
